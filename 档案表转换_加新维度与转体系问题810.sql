...
from
     (select jf.交付达标id,min(交付周期) 交付周期  
      from 
      (select jr.org_id,
              case when 
              ...
              case when 
              sum(jr.金融支付单) over( partition by jr.org_id ORDER BY jr.sj)/sum(jr.支付单) over( partition by jr.org_id ORDER BY jr.sj) >=0.5
              then  to_date(jr.sj,'yyyy-mm-dd')-to_date(to_char(u.CREATED_TIME, 'yyyy-mm-dd'),'yyyy-mm-dd') end as 交付周期             
              from                       
                      ...
                      left join ods.T_TAX_WAYBILL tw on tw.TAX_WAYBILL_ID = b.TAX_WAYBILL_ID
                     where a.IS_DEL = 0
                         and a.PAY_TIME <= u.CREATED_TIME +30 and a.PAY_TIME >= u.CREATED_TIME --and a.PAY_TIME >= u.CREATED_TIME +7
                       --and a.PAY_TIME >= to_date('2020-04-01', 'yyyy-mm-dd')
                       --and a.PAY_TIME < to_date('2020-07-01', 'yyyy-mm-dd')
       ...
       where jr.sj >= to_char(u.CREATED_TIME+7, 'yyyy-mm-dd') and sysdate >= u.CREATED_TIME+7
       ) jf
      group by jf.交付达标id
      -------第七天有支付单的达标项目 -上
      -------第七天以后无支付单但是达标的项目-下
      ...
      select case
       ...
       7 交付周期
        from (select a.org_id,
          ...
          left join t_y_s u on u.org_id = a.org_id --每个项目金融付运单首次支付时间+首次建单时间
          left join ods.T_TAX_WAYBILL tw on tw.TAX_WAYBILL_ID = b.TAX_WAYBILL_ID
         where  a.PAY_TIME <= u.CREATED_TIME + 7
           and a.PAY_TIME > u.CREATED_TIME
           and sysdate >= u.CREATED_TIME + 7
           and a.PAY_STATE = 2
           and a.IS_DEL = 0
           and tw.PAY_STATE = 2
           and tw.IS_DEL = 0
...
-------将转体的id状态与新企业id的状态融合。这个表存在的原因是计算实施和运营表！！
...
from (select 新企业ID,
             ...
             min(case when 是否是供应链金融项目 = '是' then 1 else 0 end) 是否是供应链金融项目,
             max(交付周期) 交付周期,
             sum(Q2支付单) Q2支付单,
             sum(Q2金融支付单) Q2金融支付单,
             sum(Q1支付单) Q1支付单,
